...
from lib.tensor_utils import random_mask_by_proportion
...
class DatasetBase(Dataset):
    def __init__(self, 
        ...
        daughter_subset: list[str] | None, # list of daughter languages to include. the vocab will still be built from the entire dataset
            ...
            # sth like ["Italian", "French", ...] means only include those daughters. Concatinated dataset will also be in that order.
        min_daughters: int, # e.ge 3 # minimum number of daughters in a cognate set to be included in the dataset. 
            # only applicable to datasets where missing cognates are not indicated by a placeholder, such as WikiHan
        verbose: bool,
        proportion_labelled: float, # proportion of the dataset that is labelled, useful for semi-supervised learning
            # for Improved Neural Protoform Reconstruction via Reflex Prediction, this is always 1.0, as we are supervised
    ):
        ...
        p_labelled_mask = random_mask_by_proportion(self.length, proportion_labelled).bool()
...
class DatasetConcat(DatasetBase):
    def __init__(self, 
        ...
        transformer_d2p_d_cat_style: bool, # if true the concat daughter seq follow Kim et al. (2023)'s style: <bos>d1<eos><bos>d2<eos>...
    ...
    def __getitem__(self, idx) -> tuple[Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, list[dict]]:
        ...
        if self.transformer_d2p_d_cat_style:
            ...
            # NOTE d_indv_lens_tensor is the len of the entire |Mandarin|<:>|i|˧˥|<*>|. so the example has length 5.
        ...
        p_tkns = list(self.Ps[idx].values())[0]
        if p_tkns == []: # empty protoform
        ...
        # 3 > create individual daughter sequences
        ...
        for d_lang_str, d_str_seq in self.D[idx].items():
            if (len(d_str_seq) == 1) and (d_str_seq[0] == '-'): 
                continue # Kim et al. (2023) had placeholder '-' token for unknown daughter, but we don't want the reflex prediction model to learn to produce that
        ...
        return d_cat_tkns_tensor, d_cat_langs_tensor, d_indv_lens_tensor, d_cat_lens_tensor, p_tkns_tensor, p_langs_tensor, p_lens_tensor, daughters_entries_list
    ...
    def d2p_collate_fn(self, 
        ...
        for p_concat_seq_tensor, p_concat_lang_tensor, p_individual_lens_tensor, p_concat_seq_len_tensor, p_seq_tensor, p_lang_tensor, p_len_tensor, daughters_entries_list in batch:
        ...
        return d_cat_tkns, d_cat_langs, d_cat_lens, d_indv_lens, p_lang_lang_vec, p_tkns
