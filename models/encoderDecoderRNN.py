# loosely based on Chang et al. (2022) https://github.com/cmu-llab/meloni-2021-reimplementation/blob/main/model.py, with many modifications
...
class Seq2SeqRNN(pl.LightningModule):
    def __init__(self,
        ...
        self.lang_embedding_when_decoder = lang_embedding_when_decoder # should have been called lang_embedding_when_decoding...
        ...
        self.enable_logging = True # Backward compatibility
    ...
    def beam_search_decode(self, 
        ...
        beam_log_probs = torch.zeros(B * N).to(self.device) # (BN)
        ...
        match torch.all(tkn_pred_eos_mask == False).item():
            case True: # nothing reached EOS, no endnodes to add
                pass 
            case False: # some reached EOS, add endnodes
        ...
        for i in range(self.inference_decode_max_length):
            ...
            beam_log_probs = 0 + rearrange(log_probs, 'N B -> (B N)') 
            dbg('beam_sequences', beam_sequences)
            dbg('beam_log_probs', beam_log_probs)        
    ...
    def rerank_endnodes(self, 
        ...
        **reranker_kw_args,
        ...
        truncate_thresh = 9999 # effectively no truncation
    ...
    def teacher_forcing_decode(self, 
        ...
        N = source_tokens.size(0) # batch size
    ...
    def unpack_batch(self, batch) -> tuple[Tensor, Tensor | None, Tensor, Tensor, Tensor, Tensor]:
        match self.training_mode:
            case 'encodeWithLangEmbedding':
                (d_cat_tkns, d_cat_langs, d_cat_lens, d_indv_lens, p_lang_lang_vec, p_tkns), _, _ = batch   
    ...
    def reunpack_batch_for_reranking(self, batch):
        ...
        # WARNING: this assumes the batch[0] is from d2p and batch[1] is from p2d. it also assumes N = 1 for the d2p component.
        (daughters_concat_seq, daughters_concat_langs, daughters_concat_seqs_lens, _d_indv_lens, _p_lang_lang_vec, proto_seq), (prompted_proto_seq, prompted_proto_seqs_lens, target_ipa_langs, target_lang_langs, daughter_seqs), _ = batch
    ...
    def rereunpack_batch_for_batched_reranking(self, batch):
        assert self.training_mode == 'encodeWithLangEmbedding' # make sure it's d2p
        # WARNING: this assumes the batch[0] is from d2p and batch[1] is from p2d. it also assumes N = 1 for the d2p component.
        (daughters_concat_seq, daughters_concat_langs, daughters_concat_seqs_lens, _d_indv_lens, _p_lang_lang_vec, proto_seq), (prompted_proto_seq, prompted_proto_seqs_lens, target_ipa_langs, target_lang_langs, daughter_seqs), (prompted_proto_seqs_s, prompted_proto_seqs_lens_s, daughters_ipa_langs_s, daughters_lang_langs_s, daughters_seqs_s) = batch
    ...
    def get_loss_from_teacher_forcing_decode_res(self, decode_res, target_tokens):
        if self.use_vae_latent:
            ...
            recon_loss = utils.calc_cross_entropy_loss(
                ...
                target_tokens[:, 1:] # (N, T)
            ) # looks like tensor(5.4489, grad_fn=<NllLoss2DBackward0>)
